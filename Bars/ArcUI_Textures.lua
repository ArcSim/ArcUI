...
local function StopPulse(frame)
  if frame._arcPulseAG and frame._arcPulseAG:IsPlaying() then
    frame._arcPulseAG:Stop()
  end
end

local function StartPulse(frame, cfg)
  local d = cfg.display
  local ag = frame._arcPulseAG
  if not ag then
  ...
  if a.SetScaleFrom and a.SetScaleTo then
    a:SetScaleFrom(1, 1)
    a:SetScaleTo(target, target)
  elseif a.SetScale then
    a:SetScale(target, target)
  end
  a:SetDuration(dur)
...
-- ===================================================================
-- GLOW (reuses ns.Glows, the shared glow renderer)
-- ===================================================================
local GLOW_TYPE_MAP = { pixel = "pixel", autocast = "autocast", button = "button", proc = "proc" }

local function StopGlow(frame)
  if frame._arcGlowOn and ns.Glows and ns.Glows.Stop then
    ns.Glows.Stop(frame, "arcTex")
  end
  frame._arcGlowOn = false
end

local function StartGlow(frame, cfg)
  if not (ns.Glows and ns.Glows.Start) then return end
  local gt = GLOW_TYPE_MAP[cfg.display.glowType or "pixel"] or "pixel"
  ns.Glows.Start(frame, "arcTex", gt, { color = { 1, 0.82, 0, 1 } })
  frame._arcGlowOn = true
end
...
local function EnsureFrame(num)
  ...
  dtf:SetSize(40, 20)
  local dtext = dtf:CreateFontString(nil, "OVERLAY")
...
local function EnsureGrips(num, frame, cfg)
  if not frame._arcGrips then
    ...
    -- Editor-only drain-region outline (cyan rectangle) shown when the region is inset.
    frame._arcRegionLines = {}
    for i = 1, 4 do
      local ln = frame:CreateTexture(nil, "OVERLAY")
...
function Textures.UpdateTexture(num)
  ...
  -- Pulse + glow show whenever the ACTIVE appearance is shown -- including the
  -- editing preview (useActiveStyle) so toggling them updates live in the panel.
  -- The size pulse still pauses during an active resize drag so it doesn't fight
  -- the corner grips.
  ...
  if useActiveStyle and (d.glowEnabled == true) then
    StartGlow(frame, cfg)
  else
    StopGlow(frame)
  end
